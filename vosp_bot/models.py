...
class Vosp(models.Model):
    ...
    lunch_duty_day = models.DateField(
        verbose_name="Дата дежурства на ужин", blank=True, null=True, unique=True
    )
    tea_duty_day1 = models.DateField(
        verbose_name="Дата первого дежурства к чаю", blank=True, null=True, unique=True
    )
    tea_duty_day2 = models.DateField(
        verbose_name="Дата второго дежурства к чаю", blank=True, null=True, unique=True
    )
    ...
    def change_day(self, val=1):
        if (self.lunch_duty_day + datetime.timedelta(days=val)).isoweekday() not in (
            5,
            6,
        ):
...
class Mutfak(models.Model):
    ...
    vosp1 = models.ForeignKey(
        Vosp, on_delete=models.CASCADE, related_name="duty1", verbose_name="Дежурный 1"
    )
    vosp2 = models.ForeignKey(
        Vosp, on_delete=models.CASCADE, related_name="duty2", verbose_name="Дежурный 2"
    )
