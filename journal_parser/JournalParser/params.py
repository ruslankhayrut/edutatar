...
class Params:
    ...
    def set_attributes(self, kwargs):
        for key, val in kwargs.items():
            if key in TO_FLOAT:
                ...
                for e in range(len(allowed_not_row)):
                    allowed_not_row[e] = (
                        allowed_not_row[e][0].upper() + allowed_not_row[e][1:].lower()
                    )
        ...
        if (
            not self.check_RO
            and not self.check_meta
            and not self.check_lessons_fill
            and not self.check_students_fill
            and not self.check_double_two
        ):
            self.only_term = True
        else:
