...
import locale
import datetime
from .params import CONTROL_WORKS, MARKS_COLUMN_TYPES, MARKS
...
def fill_checker(sheet, page_data, params):
    ...
    if not params.only_term:
        for i in range(len(days)):
            ...
            if params.check_RO and l_type in CONTROL_WORKS and days[i] != next_day and next_l_type != 'РО':
            ...
            if params.check_lessons_fill:
                ...
                if l_type in MARKS_COLUMN_TYPES and not (l_type == 'ПР' and subject.startswith(params.allowed_not_row)):
                ...
                if (l_type not in MARKS_COLUMN_TYPES or (l_type == 'ПР' and subject.startswith(params.allowed_not_row))) \
                        and round(mrks / students_count, 2) < params.lesson_percent / 100 - 0.01:
