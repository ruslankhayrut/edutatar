...
import locale
import datetime
from .constants import CONTROL_WORKS, MARKS_COLUMN_TYPES, MARKS
...
def fill_checker(sheet, page_data, params):
    ...
    if term_marks and params.get('check_term_marks'):

        MINIMAL_FOR_5 = params.get('min_for_5', 0)
        MINIMAL_FOR_4 = params.get('min_for_4', 0)
        MINIMAL_FOR_3 = params.get('min_for_3', 0)

        for i in range(len(average_marks)):
            if average_marks[i] and term_marks[i]:
                if average_marks[i] >= MINIMAL_FOR_5 and term_marks[i] != 5:
                    date = ''
                    w = 'Несоответствие средней и четвертной оценок. Строка {}'.format(i+1)
                    pattern = row_pattern + [date, w]
                    warnings.append(pattern)

                elif MINIMAL_FOR_4 <= average_marks[i] < MINIMAL_FOR_5 and term_marks[i] != 4:
                    date = ''
                    w = 'Несоответствие средней и четвертной оценок. Строка {}'.format(i + 1)
                    pattern = row_pattern + [date, w]
                    warnings.append(pattern)

                elif MINIMAL_FOR_3 <= average_marks[i] < MINIMAL_FOR_4 and term_marks[i] != 3:
                    date = ''
                    w = 'Несоответствие средней и четвертной оценок. Строка {}'.format(i + 1)
                    pattern = row_pattern + [date, w]
                    warnings.append(pattern)
    ...
    for i in range(len(days)):
        ...
        if params.get('check_RO') and l_type in CONTROL_WORKS and days[i] != next_day and next_l_type != 'РО':
            w = 'После КР или Диктанта не работа над ошибками'
            pattern = row_pattern + [date, w]
            warnings.append(pattern)

        """Validating lesson topic and task"""
        if params.get('check_meta') and l_meta:
            if not l_meta[0].startswith('Тема') or not l_meta[1].startswith('Задание'):
        ...
        """Filling validator"""
        if params.get('check_lessons_fill'):
            mrks = 0
            filled = 0
            for row in marks:
                if row[i]:
            ...
            if l_type in MARKS_COLUMN_TYPES and not subject.startswith(allowed_not_row):
                if filled != students_count and days[i] + datetime.timedelta(days=8) < datetime.date.today():
                    w = 'Должен быть ряд оценок'
                    pattern = row_pattern + [date, w]
                    warnings.append(pattern)

            """Regular lessons fill validator"""
            if (l_type not in MARKS_COLUMN_TYPES or (l_type == 'ПР' and subject.startswith(allowed_not_row))) \
                    and round(mrks / students_count, 2) < int(params.get('lesson_percent', 0)) / 100 - 0.01:
                if l_type in ('Р', 'П') and days[i] + datetime.timedelta(days=8) >= datetime.date.today():
                    pass
                else:
    ...
    if params.get('check_students_fill') or params.get('check_double_two'):
