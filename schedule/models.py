...
class Lesson(models.Model):
    ...
    @property
    def lesson_end(self):
        return (datetime.datetime.combine(datetime.date(1, 1, 1), self.start_time) + datetime.timedelta(minutes=self.duration)).time()

    def __str__(self):
        return 'Урок {0} | {1} - {2}'.format(self.number, self.start_time, self.lesson_end)
...
class Day(models.Model):
    number = models.PositiveSmallIntegerField(verbose_name='День недели (1-7)', unique=True,
                                              validators=[MaxValueValidator(7, 'Введите корректный номер дня'),
    ...
    def weekday_str(self):
        if self.number:
            return ('Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Суббота', 'Воскресенье')[self.number - 1]
        return ''
