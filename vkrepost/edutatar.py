...
class MenuUploader:
    ...
    def post_page(self, session, page_id, data):
    ...
    def get_files(self, session, from_folder="food"):
    ...
    def upload_file(self, session, file_path, target_folder):
        data = {}
        img = open(file_path, mode="rb")
        filename = img.name.split("\\")[-1]
        data[filename] = img
        f = self.upload_files(session, data, target_folder)
        img.close()
        return f
